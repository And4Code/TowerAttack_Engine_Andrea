...
public class MapManager : MonoBehaviour
{
    ...
    // Appel du script entity manager pour la condition de destuction des outpost
    ...
    public void Awake()
    {
        // Création des deux zones (Player et Ennemy)
    ...
    private void Update()
    {
        ...
        //Petite tour Ennemy de droite détruite
        if (!entityManager.ennemyRightOutPost)
        {
            //Libère la drop zone sur la droite de la zone ennemie (le vecteur 3 prend  7 en x car c'est le centre de la map sur cet axe)
            SetAlignementZone(Alignment.Player, new Vector3(7, 0, mapData.height / 2), mapData.width / 2 - 1, mapData.height / 2);

        }

        //Petite tour Enemy de gauche détruite
        if (!entityManager.ennemyLeftOuptost)
        {
            //Libère la drop zone sur la gauche de la zone ennemie
